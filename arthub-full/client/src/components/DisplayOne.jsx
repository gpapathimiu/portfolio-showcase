        ...
        const fetchData = async () => {
            try {
                const [postResponse, authorResponse, userResponse] = await Promise.all([
                    ...
                    axios.get(`http://localhost:8000/api/user/${post.user}`, { withCredentials: true }),
                    axios.get(`http://localhost:8000/api/user/${userId}`, { withCredentials: true }),
                ]);

                setPost(postResponse.data.post);
                setAuthorName(authorResponse.data.user.username);
                const userLiked = postResponse.data.post.likes && postResponse.data.post.likes.includes(userId);
    ...
    const handleSubmitComment = (e) => {
        ...
        axios
            .post(
                ...
                { text: commentText, user: userId, userName: authorName },
                { withCredentials: true }
            )
            .then((res) => {
        ...
        <div className="max-w-3xl mx-auto mt-10">
            <div className="p-8 shadow-md rounded-md flex relative">
                ...
                <div className="w-2/5 relative">
                    <h2 className="text-2xl font-medium text-blue-700 mb-4">{post ? `Title: ${post.title}` : "This is a title"}</h2>
                    <div>
                        <p>Created by: {authorName ? authorName : "Loading..."}</p>
                    </div>
                    <p className="text-gray-600 mb-4">{post ? `Description: ${post.description}` : "This is a description"}</p>
                    <div className="flex justify-between items-center mb-4">
                        <p className="text-gray-500 mx-auto">Likes: {post && post.likes ? post.likes.length : 0}</p>
                    </div>
                    <div className="flex flex-row items-center gap-4">
                        <div className="flex flex-row items-center gap-4 mx-auto my-auto">
                            <button
                                onClick={isLiked ? handleDislike : handleLike}
                                className={`${!isLiked ? 'bg-green-500 hover:bg-green-400' : 'bg-red-500 hover:bg-red-400'} border-none rounded-md text-white font-semibold w-20 h-10 flex items-center justify-center`}
                            >
                                {isLiked ? "Dislike" : "Like"}
                            </button>
                            <button className="bg-blue-800 hover:bg-orange-400 border-none rounded-md text-white font-semibold w-20 h-10 flex items-center justify-center">Share</button>
                            {userId == post.user ? (
                                <button
					...
					<div className="my-12">
							<QRCodeSVG value={`http://localhost:5173${location.pathname}`} className="mx-auto" />
						</div>
                    <form className="flex items-center mt-4 absolute bottom-12" onSubmit={handleSubmitComment}>
                        <input
                            type="text"
